...
def get_tokenizer(opt):
    ...
    print_rank_0(f"Llama tokenizer pad token: {tokenizer.pad_token}, pad_token_id: {tokenizer.pad_token_id}",
                 only_on_cuda0=True)
...
class OnlyPromptDataset(IterDataset):
    ...
    def format(self, sample: List[str]) -> Dict[str, Any]:
        ...
        context_vec = self.tokenizer.encode(get_model_prompt(context, self.tokenizer.eos_token, self.opt),
    ...
    def batchify(self, batch_samples: List[Dict[str, Any]]) -> Dict[str, Any]:
        batch = {
            ...
            'text_vec': torch.tensor(
                pad_sequences([sample['text_vec'] for sample in batch_samples], pad_value=self.tokenizer.pad_token_id),
                dtype=torch.long),
            ...
            'loss_mask': torch.tensor(pad_sequences([sample['loss_mask'] for sample in batch_samples], pad_value=0),
                                      dtype=torch.bool),
