...
class RLHFTrainableModelWrapper(nn.Module):
    ...
    def forward(self, inputs, **kwargs):
        return self.policy_model(decoder_input=inputs, **kwargs), self.critic_model(decoder_input=inputs,
...
class PPOTrainer():
    def __init__(self, opt, policy_model, ref_model, critic_model, reward_model, accelerator, **kwargs) -> None:
        ...
        self.model, self.optimizer, self.scheduler = self.accelerator.prepare(self.model, self.optimizer,
                                                                              self.scheduler)
        ...
        for _context, _response in zip(context, responses):
            ...
            for _, resp in _response:
                ...
                if resp[-1] != self.tokenizer.eos_token_id:
                    logging.warn(
                        f'Generated response is too long: {self.tokenizer.decode(_context + resp, skip_special_tokens=False)}')
    ...
    def make_experiences(self):
        ...
        while len(self.replay_buffer) < self.num_rollouts:
            ...
            context_vec_sampled, resp_vec_sampled, sampled_vec = self.concat_context_and_response(context_vec,
                                                                                                  responses_vec)
            ...
            assert ref_logits.size(1) == logits.size(1) == values.size(
                1), f'{ref_logits.size()}, {logits.size()}, {values.size()}'
    ...
    def criterion(self, model_output: Tuple[torch.Tensor, ...], batch: Dict[str, Any], return_output=False,
                  training=True):
